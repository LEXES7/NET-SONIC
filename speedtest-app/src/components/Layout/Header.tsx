...
import { Menu, X, Moon, Sun, Github, HelpCircle, Info, Activity, Zap } from 'lucide-react';

export default function Header() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isDarkMode, setIsDarkMode] = useState(true); // Default to dark mode for cyberpunk theme
  ...
  useEffect(() => {
    ...
    if (storedTheme) {
      ...
      // Always prefer dark mode for our cyberpunk theme
      setIsDarkMode(true);
  ...
  return (
    <header 
      className={`fixed w-full top-0 z-50 transition-all duration-300 backdrop-blur-md ${
        scrolled 
          ? 'bg-black/50 shadow-[0_0_15px_rgba(252,238,9,0.15)]' 
          : 'bg-black/30'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <nav className="hidden md:flex items-center space-x-1">
            <Link 
              href="/" 
              className="relative px-4 py-2 text-sm font-medium text-yellow-400/80 hover:text-yellow-400 transition-colors rounded-md group"
              style={{
                fontFamily: "'Share Tech Mono', monospace"
              }}
            >
              <span className="relative z-10">HOME</span>
              <span className="absolute inset-0 bg-yellow-400/10 rounded-md scale-0 group-hover:scale-100 transition-transform duration-300 origin-left"></span>
            </Link>
            ...
            {/* Theme toggle button */}
            <div className="pl-2 border-l border-yellow-400/20 ml-2">
              <button 
                onClick={toggleDarkMode} 
                className={`p-2.5 rounded-full transition-all duration-300 ${
                  isDarkMode 
                    ? 'bg-black/50 hover:bg-black/70 text-yellow-400 hover:text-yellow-300'
                    : 'bg-yellow-400/10 hover:bg-yellow-400/20 text-yellow-400 hover:text-yellow-300'
                }`}
                style={{
                  boxShadow: isDarkMode ? '0 0 10px rgba(252, 238, 9, 0.2)' : 'none'
                }}
      ...
      <div 
        id="mobile-menu" 
        className={`md:hidden absolute top-full left-0 w-full bg-black/80 backdrop-blur-md border-t border-yellow-400/20 shadow-[0_5px_15px_rgba(0,0,0,0.5)] transition-all duration-300 transform ${
      ...
      >
        <div className="px-2 py-3 space-y-1">
          <Link 
            href="/" 
            className="flex items-center px-3 py-2.5 text-base font-medium text-yellow-400 rounded-md hover:bg-yellow-400/10"
            style={{
              fontFamily: "'Share Tech Mono', monospace"
            }}
